...
API.call().get(`articles/popular?limit=4`).then((response) => {
  ...
  articles.forEach((item) => {
    ...
    <div class="swiper-slide">
                  <a href="detail.html?id=${item.id}" class="img-bg d-flex align-items-end"
...
API.call().get(`articles/popular?limit=5`).then((response) => {
  const articles = response.data.data;
...
API.call().get(`articles?limit=7`).then((response) => {
  const articles = response.data.data;
...
API.call().get(`categories_news/articles?limit_cate=4&limit=9`).then((response) => {
...
API.call().get(`categories_news/articles?limit_cate=4&limit=4`).then((response) => {
  const data = response.data.data;
  let htmlTab = '';
  let htmlTabContent = '';
  data.forEach((item, index) => {
      ...
      htmlArticle += /*html*/ `
           <div class="col-md-6 col-lg-3">
                <div class="post-entry-1">
                  <a href="detail.html?id=${articleItem.id}"><img src="${articleItem.thumb}" alt="${articleItem.title}" class="img-fluid" /></a>
                  <div class="post-meta">
    ...
    htmlTabContent +=/*html*/`
      <div class="tab-pane fade ${activeShow}" id="${slug}-tab-pane" role="tabpanel" aria-labelledby="${slug}-tab" tabindex="0">
          <div class="row g-5">${htmlArticle}</div>
      </div>`
...
function renderCategorySectionTitle(categoryName) {
  return /*html*/`
    ...
    <h2>${categoryName}</h2>
    <div><a href="category.html" class="more">See All ${categoryName}</a></div>
  </div>`
}
...
function renderArticlesByCategoryFeatured(articles, idx) {
  ...
  articles.forEach((articleItem, index) => {
    ...
    if (index < 4) {
      htmlArticleLeft +=/*html*/
        `<div class="col-lg-6">
                <div class="post-entry-1">
                  <a href="detail.html?id=${articleItem.id}"><img src="${thumb}" alt="${title}" class="img-fluid" /></a>
                  <div class="post-meta">
                    <span>${publishDate}</span>
                  </div>
  ...
  let rowClass = '';
  let borderClass = 'border-start custom-border';
  ...
  return /*html*/ `
        <div class="row g-5 ${rowClass}">
          <div class="col-lg-8">
            <div class="row g-5">${htmlArticleLeft}</div>
          </div>
          <div class="col-lg-4 ${borderClass}">${htmlArticleRight}</div>
        </div>
      </div>`;
}
